...
### 12.1 Referência SQL - schema atual do banco
...
-- ==============================================
--
-- Não execute este bloco automaticamente no banco do cliente.
-- Use apenas para comparar metadata e gerar migrations revisáveis.
--
-- Estruturas já existentes no banco informado:
...
-- - TBINSPECAO_MANUAL já existe para as inspeções manuais deste projeto.
...
### 12.2 Validação quando a tabela já existir

No banco informado, a `TBINSPECAO_MANUAL` já existe. Não recrie a tabela e não aplique comandos de ajuste sem backup e revisão manual. Use consultas como estas para comparar o metadata antes de gerar uma nova migration:
...
### 12.3 Como executar scripts revisados

```bash
# Usando isql (Firebird Interactive SQL)
isql -user SYSDBA -password masterkey /caminho/para/database.fdb -i docs/firebird-integration/migrations/arquivo_revisado.sql
...
# Validar tabelas existentes no banco
# Compare o metadata com docs/firebird-integration/08-face-id-database-schema.md
